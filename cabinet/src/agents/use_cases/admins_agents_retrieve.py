...
class AdminsAgentsRetrieveCase(BaseAgentCase):
    """
    ...
    async def __call__(self, agent_id: int) -> User:
        active_filters: dict[str, Any] = dict(
            active=True,
            agent_id=agent_id,
        ...
        active_booking_qs: Any = self.booking_repo.exists(filters=active_filters)
        succeed_filters: dict[str, Any] = dict(
            active=True,
            agent_id=agent_id,
        ...
        succeed_booking_qs: Any = self.booking_repo.exists(filters=succeed_filters)
        closed_filters: dict[str, Any] = dict(
            active=True,
            agent_id=agent_id,
            user_id=self.user_repo.a_builder.build_outer("id"),
            amocrm_substage__in=[
        ...
        closed_booking_qs: Any = self.booking_repo.exists(filters=closed_filters)
        ...
        filters: dict[str, Any] = dict(
            agent_id=agent_id, type=self.user_types.CLIENT, is_deleted=False
        )
        users_qs: Any = self.user_repo.list(filters=filters, prefetch_fields=prefetch_fields)

        filters: dict[str, Any] = dict(id=agent_id, type=self.user_types.AGENT)
        prefetch_fields: list[Any] = [
            dict(relation="users", queryset=users_qs, to_attr="clients"),
            "agency",
        ]
        annotations: dict[str, Any] = dict(
