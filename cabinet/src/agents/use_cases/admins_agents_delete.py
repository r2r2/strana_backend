...
class AdminsAgentsDeleteCase(BaseAgentCase):
    """
    ...
    def __init__(
        ...
        self.agent_update = user_changes_logger(
            self.agent_repo.update, self, content="Удаление агента администратором | Помечаем агента удалённым"
        )
        self.user_repo: AgentUserRepo = user_repo()
        self.user_update = user_changes_logger(
            self.user_repo.update, self, content="Отвязываем пользователей от агента"
        )
    ...
    async def __call__(self, agent_id: int) -> User:
        filters: dict[str, Any] = dict(id=agent_id, is_deleted=False, type=self.user_types.AGENT)
        agent: User = await self.agent_repo.retrieve(filters=filters)
        if not agent:
            raise AgentNotFoundError

        # Маркируем агента удалённым
        data = dict(is_deleted=True)
        agent = await self.agent_update(agent, data=data)

        # Отвязываем пользователей от агента
        filters = dict(agent_id=agent.id, type=self.user_types.CLIENT)
        ...
        # Удаляем агента из бронирований
        filters = dict(agent_id=agent.id)
