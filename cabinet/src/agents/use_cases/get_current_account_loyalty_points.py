...
class GetCurrentAccountLoyaltyPointsCase(BaseAgentCase):
    ...
    async def __call__(self, agent_amocrm_id: int, payload: LoyaltyStatusRequestModel) -> None:
        ...
        if payload.booking and payload.booking.amocrm_id and payload.booking.point_amount is not None:
            booking: Booking = await self.booking_repo.retrieve(
                filters=dict(amocrm_id=payload.booking.amocrm_id),
                related_fields=["property"],
            )
            if not booking:
                raise BookingNotFoundError

            if booking.property.type not in [PropertyTypes.PANTRY, PropertyTypes.PARKING]:
