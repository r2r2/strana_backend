...
from ..exceptions import (AgentHasNoSignupEventError,
                          EventHasAlreadyEndedError, EventNotFoundError)
from ..repos import (Event, EventParticipant, EventParticipantRepo,
                     EventParticipantStatus, EventRepo, EventType)


class EventAgentRefuseCase(BaseEventCase):
    """
    Кейс для отказа агента от мероприятия.
    """

    mail_event_slug = "agent_refuse_from_event"
    ...
    async def __call__(
        ...
        if not event.agent_recorded:
            raise AgentHasNoSignupEventError
        ...
        event_participant: EventParticipant = await self.event_participant_repo.retrieve(
            filters=dict(
                agent_id=user.id,
                event_id=event.id,
            )
        ...
        await self.event_participant_repo.update(
            model=event_participant,
            data=dict(status=EventParticipantStatus.REFUSED)
        )

        event.agent_recorded = False
    ...
    async def _send_email_to_agent(
        ...
        **context,
        ...
        """
        Уведомляем агента о том, что он отказался от мероприятия.
