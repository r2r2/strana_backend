...
__all__ = ('AgentBookingsSpecsCase', 'AgentBookingsFacetsCase', 'AgentBookingsLookupCase')


class AgentBookingsSpecsCase(BaseUserCase):
    """
    ...
    async def __call__(self, agent_id: int, specs: Callable[..., Coroutine]) -> dict[str, Any]:
        filters: dict[str, Any] = dict(agent_id=agent_id, user__type=UserType.CLIENT)
        result: dict[str, Any] = await specs(group=self.group, filters=filters)
        return result


class AgentBookingsFacetsCase(BaseUserCase):
    """
...
class AgentBookingsLookupCase(BaseUserCase):
    """
