...
__all__ = ('RepresBookingsSpecsCase', 'RepresBookingsFacetsCase', 'RepresBookingsLookupCase')


class RepresBookingsSpecsCase(BaseUserCase):
    """
    ...
    async def __call__(self, agency_id: int, specs: Callable[..., Coroutine]) -> dict[str, Any]:
        filters: dict[str, Any] = dict(agency_id=agency_id, user__type=UserType.CLIENT)
        result: dict[str, Any] = await specs(group=self.group, filters=filters)
        return result


class RepresBookingsFacetsCase(BaseUserCase):
    """
    Фасеты пользователей представителя агенства
    """
...
class RepresBookingsLookupCase(BaseUserCase):
    """
    Поиск пользователя представителя агенства
    """
