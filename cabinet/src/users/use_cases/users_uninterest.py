...
class UsersUninterestProfitIdCase(UsersUninterestCase):
    """
    Удаление интересующих объектов пользователю по profitbase_id.
    """
    ...
    async def __call__(
            self,
            user_id: int,
            uninterested_profitbase_ids: list[int],
            agency_id: int = None,
            agent_id: int = None,
    ) -> User:
        self.init_user_data(agent_id=agent_id, agency_id=agency_id)
        ...
        filters: dict[str, Any] = dict(profitbase_id__in=uninterested_profitbase_ids)
        uninterested: list[UserProperty] = await self.property_repo.list(filters=filters)
        interested_ids: list[int] = list(map(lambda x: x.id, list(user.interested)))
