...
from ..repos import UserRepo, User, CheckRepo
from ..types import UserPropertyRepo, UserProperty, UserBooking, UserBookingRepo


class AdminsAgentsUsersRetrieveCase(BaseUserCase):
    """
    ...
    async def __call__(self, user_id: int, agent_id: int) -> User:
        ...
        filters: dict[str, Any] = dict(id=user_id, checkers=agent_id, type=UserType.CLIENT)
        ...
        if user.agent_id and int(user.agent_id) == int(agent_id):
            ...
            filters: dict[str, Any] = dict(user_id=user_id, active=True, agent_id=agent_id)
            bookings: list[UserBooking] = await self.booking_repo.list(
                filters=filters, related_fields=["floor", "project", "building", "property"]
            )
            user: User = self.user_repo.c_builder.denormalize(
                bases=user,
                update=True,
                to="interesting",
                objects=properties,
