...
class GenerateAgentsUsers(object):
    """
    Генерация тестовых пользователей для агента
    """
    ...
    async def __call__(self, *args, **kwargs):
        ...
        if self.email_flag in argv and self.quantity_flag in argv:
            ...
            if len(argv) >= email_index + 2:
                ...
                agent: agents_repos.User = await self.agent_repo.retrieve(filters=filters)
                if agent:
                    if agent.type != users_constants.UserType.AGENT:
                        data: dict[str, Any] = dict(type=users_constants.UserType.AGENT)
                        await self.agent_repo.update(agent, data=data)
                    self._phone: str = agent.phone
                    ...
                    for i in range(quantity):
                        user: users_repos.User = await self.generate_main(agent=agent)
    ...
    @skip_integrity
    async def generate_main(self, agent: agents_repos.User) -> users_repos.User:
        return await self._generate_user(agent=agent)
