...
class AmoCRMAvailableMeetingSlots:
    ...
    def __init__(
        ...
        self._session: ClientSession = (
            ClientSession(connector=TCPConnector(verify_ssl=False))
            if maintenance_settings.get("environment", "dev")
            else ClientSession()
        )

    async def __call__(
        self, city_slug: str, meet: list[str], project_id: int | None = None
    ) -> list[dict[str:Any]]:
        if project_id:
    ...
    def _transform_response(self, response: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        transformed_data = [
            {"date": date_, "times": times} for date_, times in grouped_data.items()
        ]
    ...
    def custom_slots(self):
        ...
        available_slots = []
        for day in day_list:
            slot: dict[str, str] = dict(
                date=day.strftime("%Y-%m-%d"), times=self._time_slot
            )
