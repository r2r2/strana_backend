...
from common.filters import (BooleanFilter, FacetFilterSet, FloatFilter,
                            IntegerFilter)
...
class OfferFilterSet(GrapheneFilterSetMixin, FacetFilterSet):
    """
    Фильтр предложений
    """

    type = ChoiceFilter(field_name="type", choices=MortgageType.choices, label="Фильтр по типу")
    program = GlobalIDFilter(field_name="program", label="Фильтр по программе")
    ...
    project = GlobalIDMultipleChoiceFilter(field_name="projects__slug", label="Фильтр по проекту")
    city = GlobalIDFilter(field_name="city", label="Фильтр по городу")
    is_site = BooleanFilter(field_name="is_site", label="Фильтр на сайте")
        ...
        facets["facets"].append(
            {
                "name": "price",
                "range": {
                    "min": values["min"] and math.ceil(values["min"]),
                    "max": values["max"] and math.floor(values["max"]),
                },
            }
        )
        ...
        specs = [
            {
                ...
                "value": to_global_id("ProgramType", i["program__id"]),
