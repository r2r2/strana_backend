...
class Building(models.Model):
    ...
    total_floor = models.IntegerField(
        blank=True, null=True, verbose_name="Всего этажей"
    )
    default_commission = models.DecimalField(
        ...
        blank=True,
        null=True,
        verbose_name="Комиссия по умолчания (%)",
    )
    discount = models.PositiveSmallIntegerField(default=0, verbose_name="Скидка (%)")
    show_in_paid_booking: bool = models.BooleanField(
        verbose_name="Отображать в платном бронировании", default=True
    )
    booking_types = models.ManyToManyField(
        verbose_name="Типы бронирований",
        blank=True,
        to="properties.BuildingBookingType",
        through="properties.BuildingBookingTypeThrough",
        through_fields=("building", "booking_type"),
